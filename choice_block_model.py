...
class SAGAN_Functional_Model:
    ...
    def func_model(self, nas_choice, att_lstm_num, att_lstm_seq_len, lstm_seq_len, feature_vec_len, cnn_flat_size, lstm_out_size, \
        ...
        #1st level gate
        level = 0
        nbhd_convs = [self.conv2d_layer(nas_choice[0][level][ts][0], "nbhd_conv_time{0}_{1}".format(level, ts+1))(nbhd_inputs[ts])for ts in range(lstm_seq_len)]
        nbhd_convs = [self.pooling_layer(nas_choice[1][level][ts][0], "nbhd_pooling_time{0}_{1}".format(level, ts+1))(nbhd_convs[ts]) \
            if nas_choice[1][level][ts][0] < 6 else nbhd_convs[ts] for ts in range(lstm_seq_len)]
        nbhd_convs = [self.conv2d_activ_layer(nas_choice[2][level][ts][0], "nbhd_activ_time{0}_{1}".format(level, ts+1))(nbhd_convs[ts]) for ts in range(lstm_seq_len)]
        flow_convs = [self.conv2d_layer(nas_choice[0][level][ts][1], "flow_convs_time{0}_{1}".format(level, ts+1))(flow_inputs[ts]) for ts in range(lstm_seq_len)]
        flow_convs = [self.pooling_layer(nas_choice[1][level][ts][1], "flow_pooling_time{0}_{1}".format(level, ts+1))(flow_convs[ts]) \
            if nas_choice[1][level][ts][0] < 6 else flow_convs[ts] for ts in range(lstm_seq_len)]
        flow_convs = [self.conv2d_activ_layer(nas_choice[2][level][ts][1], "flow_activ_time{0}_{1}".format(level, ts+1))(flow_convs[ts]) for ts in range(lstm_seq_len)]
        flow_gates = [self.gate_activ_layer(nas_choice[3][level][ts], "gate_activ_time{0}_{1}".format(level, ts+1))(flow_convs[ts]) for ts in range(lstm_seq_len)]
        nbhd_convs = [keras.layers.Multiply()([nbhd_convs[ts], flow_gates[ts]]) for ts in range(lstm_seq_len)]


        #2nd level gate
        level = 1
        nbhd_convs = [self.conv2d_layer(nas_choice[0][level][ts][0], "nbhd_conv_time{0}_{1}".format(level, ts+1))(nbhd_convs[ts])for ts in range(lstm_seq_len)]
        nbhd_convs = [self.pooling_layer(nas_choice[1][level][ts][0], "nbhd_pooling_time{0}_{1}".format(level, ts+1))(nbhd_convs[ts]) \
            if nas_choice[1][level][ts][0] < 6 else nbhd_convs[ts] for ts in range(lstm_seq_len)]
        nbhd_convs = [self.conv2d_activ_layer(nas_choice[2][level][ts][0], "nbhd_activ_time{0}_{1}".format(level, ts+1))(nbhd_convs[ts]) for ts in range(lstm_seq_len)]
        flow_convs = [self.conv2d_layer(nas_choice[0][level][ts][1], "flow_convs_time{0}_{1}".format(level, ts+1))(flow_inputs[ts]) for ts in range(lstm_seq_len)]
        ...
        # poi part
        poi_convs = [Conv2D(filters = 64, kernel_size = (3, 3), padding = 'same', name = 'poi_conv_time{0}'.format(ts+1))(poi_inputs) for ts in range(lstm_seq_len)]
        poi_convs = [Activation("relu", name = "poi_conv_activation_time{0}".format(ts+1))(poi_convs[ts]) for ts in range(lstm_seq_len)]
        poi_vecs = [Flatten(name = "poi_flatten_time{0}".format(ts+1))(poi_convs[ts]) for ts in range(lstm_seq_len)]
        poi_vecs = [Dense(units = cnn_flat_size, name = "poi_dense_time{0}".format(ts+1))(poi_vecs[ts]) for ts in range(lstm_seq_len)]
        poi_vecs = [Activation("relu", name = "poi_dense_activation_{0}".format(ts+1))(poi_vecs[ts]) for ts in range(lstm_seq_len)]
        poi_vec = Concatenate(axis = -1)(poi_vecs)
        poi_vec = Reshape(target_shape = (lstm_seq_len, cnn_flat_size))(poi_vec)
        ...
        #attention part
        level = 0
        att_nbhd_convs = [[self.conv2d_layer(nas_choice[4][level][att][ts][0], "att_nbhd_convs_time{0}_{1}_{2}".format(level, att+1,ts+1))(att_nbhd_inputs[att][ts]) for ts in range(att_lstm_seq_len)] for att in range(att_lstm_num)]
        att_nbhd_convs = [[self.pooling_layer(nas_choice[5][level][att][ts][0], "att_nbhd_pooling_time{0}_{1}_{2}".format(level, att+1, ts+1))(att_nbhd_convs[att][ts]) \
            if nas_choice[5][level][att][ts][0] < 6 else att_nbhd_convs[att][ts] for ts in range(att_lstm_seq_len)] for att in range(att_lstm_num)]
        att_nbhd_convs = [[self.conv2d_activ_layer(nas_choice[6][level][att][ts][0], "att_nbhd_activ_time{0}_{1}_{2}".format(level, att+1, ts+1))(att_nbhd_convs[att][ts]) for ts in range(att_lstm_seq_len)] for att in range(att_lstm_num)]
        att_flow_convs = [[self.conv2d_layer(nas_choice[4][level][att][ts][1], "att_flow_convs_time{0}_{1}_{2}".format(level, att+1,ts+1))(att_flow_inputs[att][ts]) for ts in range(att_lstm_seq_len)] for att in range(att_lstm_num)]
        att_flow_convs = [[self.pooling_layer(nas_choice[5][level][att][ts][1], "att_flow_pooling_time{0}_{1}_{2}".format(level, att+1, ts+1))(att_flow_convs[att][ts]) 
            if nas_choice[5][level][att][ts][1] < 6 else att_flow_convs[att][ts] for ts in range(att_lstm_seq_len)] for att in range(att_lstm_num)]
        att_flow_convs = [[self.conv2d_activ_layer(nas_choice[6][level][att][ts][1], "att_flow_activ_time{0}_{1}_{2}".format(level, att+1, ts+1))(att_flow_convs[att][ts]) for ts in range(att_lstm_seq_len)] for att in range(att_lstm_num)]
        att_flow_gates = [[self.gate_activ_layer(nas_choice[7][level][att][ts], "att_gate_activ_time{0}_{1}_{2}".format(level, att+1, ts+1))(att_flow_convs[att][ts]) for ts in range(att_lstm_seq_len)] for att in range(att_lstm_num)]
        ...
        level = 1
        att_nbhd_convs = [[self.conv2d_layer(nas_choice[4][level][att][ts][0], "att_nbhd_convs_time{0}_{1}_{2}".format(level, att+1,ts+1))(att_nbhd_convs[att][ts]) for ts in range(att_lstm_seq_len)] for att in range(att_lstm_num)]
        att_nbhd_convs = [[self.pooling_layer(nas_choice[5][level][att][ts][0], "att_nbhd_pooling_time{0}_{1}_{2}".format(level, att+1, ts+1))(att_nbhd_convs[att][ts]) \
            if nas_choice[5][level][att][ts][0] < 6 else att_nbhd_convs[att][ts] for ts in range(att_lstm_seq_len)] for att in range(att_lstm_num)]
        att_nbhd_convs = [[self.conv2d_activ_layer(nas_choice[6][level][att][ts][0], "att_nbhd_activ_time{0}_{1}_{2}".format(level, att+1, ts+1))(att_nbhd_convs[att][ts]) for ts in range(att_lstm_seq_len)] for att in range(att_lstm_num)]
        att_flow_convs = [[self.conv2d_layer(nas_choice[4][level][att][ts][1], "att_flow_convs_time{0}_{1}_{2}".format(level, att+1,ts+1))(att_flow_inputs[att][ts]) for ts in range(att_lstm_seq_len)] for att in range(att_lstm_num)]
        ...
        att_poi_convs = [[Conv2D(filters = 64, kernel_size = (3, 3), padding = 'same', name = "att_poi_conv_time{0}_{1}".format(att+1, ts+1))(poi_inputs) for ts in range(att_lstm_seq_len)] for att in range(att_lstm_num)]
        att_poi_vecs = [[Flatten(name = "att_poi_flatten_{0}_{1}".format(att+1, ts+1))(att_poi_convs[att][ts]) for ts in range(att_lstm_seq_len)] for att in range(att_lstm_num)]
        att_poi_vecs = [[Dense(units = cnn_flat_size, name = "att_poi_dense_time{0}_{1}".format(att+1, ts+1))(att_poi_vecs[att][ts]) for ts in range(att_lstm_seq_len)] for att in range(att_lstm_num)]
        att_poi_vecs = [[Activation("relu", name = "att_poi_dense_activation_time_{0}_{1}".format(att+1, ts+1))(att_poi_vecs[att][ts]) for ts in range(att_lstm_seq_len)] for att in range(att_lstm_num)]
        att_poi_vec = [Concatenate(axis = -1)(att_poi_vecs[att]) for att in range(att_lstm_num)]
        att_poi_vec = [Reshape(target_shape = (att_lstm_seq_len, cnn_flat_size))(att_poi_vec[att]) for att in range(att_lstm_num)]
        ...
        att_lstm_input = [Concatenate(axis=-1)([att_lstm_inputs[att], att_nbhd_vec[att], att_weather_inputs[att], att_poi_vec[att]]) for att in range(att_lstm_num)]
        ...
        # att_cnn + att_flow + att_lstm + att_weather + short_cnn + short_flow + [short_lstm, ] + short_weather + short_poi

        inputs = flatten_att_nbhd_inputs + flatten_att_flow_inputs + att_lstm_inputs + att_weather_inputs + nbhd_inputs + flow_inputs + [lstm_inputs,] + [weather_inputs,] + [poi_inputs,]
